...
cppflags := "-I" + source_dir + " -I" + source_dir + "/vendor/sam4e/include -I" + source_dir + "/vendor/cmsis-core"
...
ldflags := cpu_flags + " -nostdlib -T" + source_dir + "/sam4e8e.ld -Wl,--gc-sections -Wl,--build-id=none -Wl,-Map,build/firmware.map"
...
build:
    mkdir -p build
    {{ cc }} {{ cppflags }} {{ cflags }} -c {{ source_dir }}/main.c -o build/main.o
    {{ cc }} {{ cppflags }} {{ cflags }} -c {{ source_dir }}/startup.c -o build/startup.o
    {{ cc }} {{ cppflags }} {{ cflags }} -c {{ source_dir }}/clock.c -o build/clock.o
    {{ cc }} {{ cppflags }} {{ cflags }} -c {{ source_dir }}/gpio.c -o build/gpio.o
    {{ cc }} {{ cppflags }} {{ cflags }} -c {{ source_dir }}/usb_cdc.c -o build/usb_cdc.o
